...
draft: false
language: en
featured_image: /images/featured/Elec.png
...
Here you can see the code for this circuit. And how it works.
    ...
    // Pin pro motorek jsme smazali

    // --- Proměnné pro časování ---

    // Pro 3-sekundový alarm
    unsigned long casVstupuDoCervene = 0; // Kdy jsme vešli do červené zóny
    const long limitAlarmu = 3000; // 3 sekundy v milisekundách

    // Pro blikání LED
    unsigned long casPoslednihoBliknuti = 0; // Kdy jsme naposled blikli
    bool stavBlikaniLED = false; // Stav blikající LED (true=svítí, false=nesvítí)
    const int intervalBlikani = 100; // Rychlost blikání (rychlé)
    ...
    void setup() {
      ...
      // pinMode pro motorek jsme smazali
    }

    void loop() {
      // Zjistíme aktuální čas na začátku smyčky
      ...
      // --- OVLÁDÁNÍ LED A ALARMU ---

      if (uhelServa <= 60) {
        // Zóna 1: (1 LED svítí) - Zelená
        digitalWrite(ledZelenaPin, HIGH);   
        digitalWrite(ledOranzovaPin, LOW);
        digitalWrite(ledCervenaPin, LOW);
        
        casVstupuDoCervene = 0; // Resetujeme časovač alarmu
        
      } else if (uhelServa > 60 && uhelServa <= 120) {
        // Zóna 2: (2 LED svítí) - Zelená + Oranžová
        digitalWrite(ledZelenaPin, HIGH);   
        digitalWrite(ledOranzovaPin, HIGH); 
        digitalWrite(ledCervenaPin, LOW);
        
        casVstupuDoCervene = 0; // Resetujeme časovač alarmu
        
      } else {
        // Zóna 3: (3 LED svítí) - Všechny (ČERVENÁ ZÓNA)
        digitalWrite(ledZelenaPin, HIGH);   
        digitalWrite(ledOranzovaPin, HIGH);  
        // Červenou LED budeme ovládat speciálně

        // --- LOGIKA ALARMU ---
        
        // Zkontrolujeme, jestli jsme právě vstoupili do červené zóny
        if (casVstupuDoCervene == 0) {
          casVstupuDoCervene = aktualniCas; // Uložíme čas vstupu
        }

        // Zkontrolujeme, jestli už uplynul limit
        if (aktualniCas - casVstupuDoCervene < limitAlarmu) {
          // Stav 1: Ještě neuplynuly 3 sekundy
          digitalWrite(ledCervenaPin, HIGH); // Červená LED jen svítí
        
        } else {
          // Stav 2: Uplynuly 3 sekundy - SPUSŤ BLIKÁNÍ
          if (aktualniCas - casPoslednihoBliknuti >= intervalBlikani) {
            casPoslednihoBliknuti = aktualniCas; // Uložíme čas bliknutí
            
            // Přepneme stav LED
            if (stavBlikaniLED == false) {
              stavBlikaniLED = true;
              digitalWrite(ledCervenaPin, HIGH);
            } else {
              stavBlikaniLED = false;
              digitalWrite(ledCervenaPin, LOW);
            }
          }
        }
      ...
      // V tomto kódu NENÍ delay(), aby mohlo časování správně fungovat
    }
